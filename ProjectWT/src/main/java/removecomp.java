...
public class removecomp extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			request.getSession().removeAttribute("message5");
			request.getRequestDispatcher("companyupdation.jsp").forward(request, response);
			}
			else {
				request.getSession().setAttribute("message5", "Company with this ID does not exist!");
				request.getRequestDispatcher("companyupdation.jsp").forward(request, response);
			}
	       }
		catch(Exception e)
		{
			request.getSession().setAttribute("message5", "Company cannot be deleted");
			request.getRequestDispatcher("companyupdation.jsp").forward(request, response);
		}
		}
